...
final class CoreDataSessionSnapshotStore: SessionSnapshotStoreProtocol {
    ...
    ///
    /// - Returns: 일치하는 세션 스냅샷이 있으면 반환하고, 없으면 nil을 반환합니다.
    ///
    /// - Throws: 읽기 작업이나 fetch 과정에서 오류가 발생하면 에러를 던집니다.
    ...
    ///
    /// - Throws: environment가 비어 있거나 쓰기 작업에 실패하면 에러를 던집니다.
    ...
    func deleteAll() async throws {
        _ = try await coreDataStack.performWrite { context in
            let request = SessionSnapshotManagedObject.fetchRequest()
...
private extension CoreDataSessionSnapshotStore {
    func fetchSnapshots(
        ...
        request.sortDescriptors = [
            NSSortDescriptor(key: #keyPath(SessionSnapshotManagedObject.environment), ascending: true)
        ]
    ...
    func fetchManagedObject(
        environment: String,
        in context: NSManagedObjectContext
    ) throws -> SessionSnapshotManagedObject? {
        let request = SessionSnapshotManagedObject.fetchRequest()
