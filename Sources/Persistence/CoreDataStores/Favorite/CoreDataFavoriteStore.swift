...
final class CoreDataFavoriteStore: FavoriteStoreProtocol {
    ...
    ///   - type: 조회할 즐겨찾기 대상 타입
    ///
    /// - Returns: 일치하는 즐겨찾기가 있으면 반환하고, 없으면 nil을 반환합니다.
    ///
    /// - Throws: 읽기 작업이나 fetch 과정에서 오류가 발생하면 에러를 던집니다.
    ...
    ///   - record: 저장할 즐겨찾기 값
    ///
    /// - Throws: id 또는 type이 비어 있거나 쓰기 작업에 실패하면 에러를 던집니다.
    ...
    func deleteAll() async throws {
        _ = try await coreDataStack.performWrite { context in
            let request = FavoriteRecordManagedObject.fetchRequest()
...
private extension CoreDataFavoriteStore {
    func fetchRecords(
        ...
        request.sortDescriptors = [
            NSSortDescriptor(key: #keyPath(FavoriteRecordManagedObject.createdAt), ascending: false),
            NSSortDescriptor(key: #keyPath(FavoriteRecordManagedObject.type), ascending: true),
            NSSortDescriptor(key: #keyPath(FavoriteRecordManagedObject.id), ascending: true)
        ]
    ...
    func fetchManagedObject(
        ...
        type: String,
        in context: NSManagedObjectContext
    ) throws -> FavoriteRecordManagedObject? {
        let request = FavoriteRecordManagedObject.fetchRequest()
