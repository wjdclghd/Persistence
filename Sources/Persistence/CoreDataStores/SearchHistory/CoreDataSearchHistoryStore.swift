...
/// - 영문 대소문자를 구분하지 않고 같은 keyword로 처리합니다.
/// - 최대 30개를 초과하면 오래된 검색 기록부터 삭제합니다.
///
/// 조회 정책
...
final class CoreDataSearchHistoryStore: SearchHistoryStoreProtocol {
    private static let maximumStoredRecordCount = 30
    ...
    ///   - record: 저장할 검색 기록 값
    ///
    /// - Throws: keyword가 비어 있거나 쓰기 작업에 실패하면 에러를 던집니다.
    func save(_ record: SearchHistoryRecord) async throws {
        ...
        _ = try await coreDataStack.performWrite { context in
            let existingManagedObjects = try self.fetchManagedObjects(
                keyword: normalizedRecord.keyword,
                in: context
            )

            if let existingManagedObject = existingManagedObjects.first {
            ...
            try self.deleteExcessRecords(in: context)
        }
    }
    ...
    func delete(keyword: String) async throws {
        ...
        _ = try await coreDataStack.performWrite { context in
            let managedObjects = try self.fetchManagedObjects(
    ...
    func deleteAll() async throws {
        _ = try await coreDataStack.performWrite { context in
            let request = SearchHistoryRecordManagedObject.fetchRequest()
...
private extension CoreDataSearchHistoryStore {
    func fetchRecords(
        ...
        request.sortDescriptors = [
            NSSortDescriptor(key: #keyPath(SearchHistoryRecordManagedObject.lastSearchedAt), ascending: false),
            NSSortDescriptor(key: #keyPath(SearchHistoryRecordManagedObject.keyword), ascending: true)
        ]
    ...
    func fetchManagedObjects(
        keyword: String,
        in context: NSManagedObjectContext
    ) throws -> [SearchHistoryRecordManagedObject] {
        let request = SearchHistoryRecordManagedObject.fetchRequest()
    ...
    func deleteExcessRecords(
        ...
        managedObjects
            .dropFirst(Self.maximumStoredRecordCount)
            .forEach(context.delete)
    }
