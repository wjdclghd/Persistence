...
/*
 SessionSnapshot과 SessionSnapshotMO 사이의 변환을 담당하는 Mapper입니다.

 Persistence API는 SessionSnapshot을 외부 계약으로 사용하고,
 Core Data 저장 계층은 SessionSnapshotMO를 사용합니다.
...
enum SessionSnapshotMapper {
    /*
     ...
     Returns:
     - SessionSnapshot 값 객체
     */
    static func toSnapshot(
        _ managedObject: SessionSnapshotMO
    ) -> SessionSnapshot {
        SessionSnapshot(
    ...
    /*
     ...
     Returns:
     - 삽입된 SessionSnapshot ManagedObject
     */
    static func insert(
        from snapshot: SessionSnapshot,
