...
public enum ProgrammaticPersistenceModel {
    ...
    ///
    /// - Returns: 기본 programmatic 모델을 생성하는 ManagedObjectModelSource
...
private extension ProgrammaticPersistenceModel {
    ...
    /// SearchHistoryRecordManagedObject를 represented class로 사용합니다.
    ...
    static func makeSearchHistoryRecordEntity() -> NSEntityDescription {
        let entity = makeEntity(
            ...
            managedObjectClassName: NSStringFromClass(SearchHistoryRecordManagedObject.self)
        )
        entity.properties = [
    ...
    ///
    /// 생성 시각과 식별자, 타입을 저장하며,
    /// FavoriteRecordManagedObject를 represented class로 사용합니다.
    /// id와 type 조합을 uniqueness constraint로 사용합니다.
    static func makeFavoriteRecordEntity() -> NSEntityDescription {
        let entity = makeEntity(
            named: "FavoriteRecord",
            managedObjectClassName: NSStringFromClass(FavoriteRecordManagedObject.self)
        )
        entity.properties = [
    ...
    /// SessionSnapshotManagedObject를 represented class로 사용합니다.
    /// environment를 uniqueness constraint로 사용합니다.
    static func makeSessionSnapshotEntity() -> NSEntityDescription {
        let entity = makeEntity(
            named: "SessionSnapshot",
            managedObjectClassName: NSStringFromClass(SessionSnapshotManagedObject.self)
        )
        entity.properties = [
            makeStringAttribute(named: "environment", isOptional: false),
            makeBoolAttribute(named: "isLoggedIn", isOptional: false, defaultValue: false),
    ...
    /// CacheEntryManagedObject를 represented class로 사용합니다.
    static func makeCacheEntryEntity() -> NSEntityDescription {
        let entity = makeEntity(
            named: "CacheEntry",
            managedObjectClassName: NSStringFromClass(CacheEntryManagedObject.self)
        )
        entity.properties = [
            makeDateAttribute(named: "createdAt", isOptional: false),
    ...
    /// SyncStateRecordManagedObject를 represented class로 사용합니다.
    /// namespace를 uniqueness constraint로 사용합니다.
    static func makeSyncStateRecordEntity() -> NSEntityDescription {
        let entity = makeEntity(
            named: "SyncStateRecord",
            managedObjectClassName: NSStringFromClass(SyncStateRecordManagedObject.self)
        )
        entity.properties = [
...
private extension ProgrammaticPersistenceModel {
    ...
    ///
    /// - Returns: 기본 설정이 완료된 NSEntityDescription
