...
public final class PersistenceContainer {
    ...
    private init(
        ...
        coreDataStack: any CoreDataStackProtocol
    ) {
    ...
    ///
    /// - Returns: 사용할 준비가 완료된 PersistenceContainer
    ///
    /// - Throws: 설정값 검증 실패, Core Data 모델 로딩 실패, persistent store 로딩 실패
    ...
    ///
    /// - Returns: 기본 bundle 모델 + live 저장소 설정으로 초기화된 PersistenceContainer
    ///
    /// - Throws: 기본 설정 생성 실패, Core Data 초기화 실패
    ...
    ///
    /// - Returns: 기본 programmatic 모델 + in-memory 저장소 설정으로 초기화된 PersistenceContainer
    ///
    /// - Throws: 코드 기반 모델 검증 실패, Core Data 초기화 실패
    ...
    ///
    /// - Returns: 검색 기록 조회, 저장, 삭제 기능을 제공하는 SearchHistoryStoreProtocol 구현체
    public func makeSearchHistoryStore() -> any SearchHistoryStoreProtocol {
    ...
    ///
    /// - Returns: 즐겨찾기 조회, 저장, 삭제 기능을 제공하는 FavoriteStoreProtocol 구현체
    public func makeFavoriteStore() -> any FavoriteStoreProtocol {
    ...
    ///
    /// - Returns: 세션 스냅샷 조회, 저장, 삭제 기능을 제공하는 SessionSnapshotStoreProtocol 구현체
    public func makeSessionSnapshotStore() -> any SessionSnapshotStoreProtocol {
        CoreDataSessionSnapshotStore(coreDataStack: coreDataStack)
    }

    /// 인증 세션 저장소 구현체를 생성합니다.
    ///
    /// 상위 계층은 구체적인 Core Data 구현체 대신
    /// AuthSessionStoreProtocol 계약을 통해 인증 세션 기능에 접근합니다.
    /// 이 메서드는 현재 컨테이너가 보관 중인 Core Data stack을 사용해
    /// 인증 세션 저장소를 조립한 뒤 반환합니다.
    ///
    /// - Returns: 인증 세션 조회, 저장, 삭제 기능을 제공하는 AuthSessionStoreProtocol 구현체
    public func makeAuthSessionStore() -> any AuthSessionStoreProtocol {
        CoreDataAuthSessionStore(coreDataStack: coreDataStack)
    }

    /// 캐시 저장소 구현체를 생성합니다.
    ///
    /// 상위 계층은 구체적인 Core Data 구현체 대신
    /// CacheStoreProtocol 계약을 통해 캐시 기능에 접근합니다.
    /// 이 메서드는 현재 컨테이너가 보관 중인 Core Data stack을 사용해
    /// 캐시 저장소를 조립한 뒤 반환합니다.
    ///
    /// - Returns: 캐시 조회, 저장, 삭제 기능을 제공하는 CacheStoreProtocol 구현체
    public func makeCacheStore() -> any CacheStoreProtocol {
        CoreDataCacheStore(coreDataStack: coreDataStack)
    }

    /// 동기화 상태 저장소 구현체를 생성합니다.
    ///
    /// 상위 계층은 구체적인 Core Data 구현체 대신
    /// SyncStateStoreProtocol 계약을 통해 동기화 상태 기능에 접근합니다.
    /// 이 메서드는 현재 컨테이너가 보관 중인 Core Data stack을 사용해
    /// 동기화 상태 저장소를 조립한 뒤 반환합니다.
    ///
    /// - Returns: 동기화 상태 조회, 저장, 삭제 기능을 제공하는 SyncStateStoreProtocol 구현체
    public func makeSyncStateStore() -> any SyncStateStoreProtocol {
        CoreDataSyncStateStore(coreDataStack: coreDataStack)
    }
}
