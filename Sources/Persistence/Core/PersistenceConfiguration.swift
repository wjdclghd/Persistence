...
public struct PersistenceConfiguration {
    ...
    ///   - migrationPlan: store 로딩 시 적용할 migration 정책
    ///
    /// - Returns: 디스크 기반 저장소를 위한 PersistenceConfiguration
    ///
    /// - Throws: 설정값이 비어 있거나 올바르지 않은 경우, 디렉터리 조회 또는 생성에 실패한 경우
    ...
    ///
    /// - Returns: 메모리 기반 저장소를 위한 PersistenceConfiguration
...
extension PersistenceConfiguration {
    ...
    ///
    /// - Throws: PersistenceError.invalidConfiguration: 필수 값이 비어 있거나 정책 조합이 잘못된 경우
...
extension PersistenceConfiguration.MergePolicyKind {
    ...
    ///
    /// - Returns: 해당 정책에 대응하는 NSMergePolicy
...
private extension PersistenceConfiguration {
    ...
    ///
    /// - Throws: PersistenceError.invalidConfiguration: 값이 비어 있거나 공백만 포함된 경우
