...
/*
 ...
 이 protocol은 NSPersistentContainer 전체를 노출하지 않고,
 저장소를 사용하는 상위 계층이 실제로 필요한 기능만 접근하도록 제한합니다.
 이를 통해 저장소 초기화, context 생성, 읽기/쓰기 실행 규칙을 모듈 내부로 숨길 수 있습니다.

 설계 의도
 - Store 구현체가 구체 타입이 아니라 계약에 의존하도록 구성
 - 테스트에서 in-memory stack으로 대체 가능하게 구성
 - 읽기와 쓰기의 실행 경로를 일관되게 유지
 */
